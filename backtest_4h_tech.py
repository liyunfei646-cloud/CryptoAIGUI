#!/usr/bin/env python3
"""
⚠️【已废弃 2026-09-24】本脚本的 days 截窗有 bug：只扣 warmup、不按 days 截窗，
   所谓 "90天" 实际覆盖 ~114 天（683 个决策点）。据此产出的长周期结论表已作废。
   → 统一改用 backtest_v31.py（SPAN_MODE=exact 为正确口径，legacy 仅用于复现旧表）。
   保留本文件仅为复现历史 / git 溯源，请勿再作为结论来源。
